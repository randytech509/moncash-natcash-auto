 ...
 * téléphone marchand lit le SMS entrant et le POST vers /api/ingest).
 *
 * ⚠️ Les gabarits exacts varient selon l'opérateur/la langue → les regex ci-dessous sont
 * volontairement tolérantes et doivent être ajustées sur un VRAI SMS. Le rapprochement (côté
 * intégrateur) exige de toute façon un txId + montant concordant avant tout auto-crédit.
 *
 * Portée depuis thie-thie-services/functions/src/lib/sms.ts (logique éprouvée sur de vrais SMS
 * MonCash/NatCash, sans aucune dépendance Firebase — pur TypeScript).
 */
...
export interface ParsedSms {
  ...
  direction: SmsDirection;     // SEUL 'in' devrait déclencher un crédit côté intégrateur
